...
public partial class ConnectionViewModel : ReactiveViewModelBase
{
    ...
    /// <summary>
    /// Разрыв связи при закрытии приложения. Подавление автоподключения здесь обязательно — без
    /// него разрыв тут же поднимает новую попытку связаться со станком, который приложение
    /// только что остановило, — но, в отличие от кнопки «Отключить», снимается сразу после
    /// разрыва: закрытие приложения не означает, что пользователь решил остаться отключённым.
    /// На Android процесс переживает смахивание из недавних, и оставленное подавление дожило бы
    /// до следующего запуска. Гонки нет: после обнуления Session событий состояния, на которых
    /// висит перезапуск автоподключения, уже не приходит.
    /// </summary>
    ...
    /// <summary>
    /// Вызывается платформой, когда приложение снова показано пользователю (Android:
    /// MainActivity.OnResume). На переиспользованном процессе startup-путь второй раз не
    /// отрабатывает, а подписка-перезапуск ждёт события от живой сессии, которой нет, — без
    /// этого вызова автоподключение после принудительного закрытия не поднимет никто.
    /// Ничего не делает, если связь уже есть, если пользователь отключился сам или если цикл
    /// подключения уже идёт.
    /// </summary>
    public Task EnsureAutoConnectAsync() =>
        Session is not null || _autoConnectSuppressed || _autoConnectCts is not null
            ? Task.CompletedTask
            : AutoConnectAsync();
