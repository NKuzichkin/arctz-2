...
*   **`ViewModels/`** — слой бизнес-логики и состояния UI.
    ...
    *   `ProgramViewModel.cs` — вью-модель главного экрана: Play/Pause/Stop и блокировка джойстиков/редактирования на время выполнения программы (`IsProgramLocked`), библиотека программ, авторинг ключевых точек, воспроизведение траектории.
...
*   **`Views/`** — слой представления (UI разметка).
    ...
    *   `MainView.axaml` (.cs) — главное представление (DataContext — `ProgramViewModel`), отображается как в `MainWindow`, так и в мобильных/браузерных точках входа. Содержит библиотеку программ, единую панель джойстиков/редактирования программы с прогрессом выполнения и модальные оверлеи (редактор точки, подтверждение, подключение).
