...
(`StatusPoller`), но ожидание пришлось развязать с зеркальным
`ConnectionViewModel.DeviceStatus`: это свойство помечено `[Reactive]` и
пропускает уведомление, если новый репорт структурно совпадает с предыдущим —
именно так ведёт себя программа с нулевой дистанцией движения (станок весь
прогон остаётся в `Idle`, `WPos` не меняется), и ожидание зависло бы навсегда,
ожидая уведомление, которого не будет. Поэтому `WaitForMotionToFinishAsync`
подписывается напрямую на недедуплицированное событие
`IDeviceSession.DeviceStatusChanged`, которое срабатывает на каждый репорт
независимо от того, отличается ли он от предыдущего.
