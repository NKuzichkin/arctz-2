...
- `AvailableEndpoints` заполняется как: реальные эндпоинты провайдера (только если
  ...
  разрешений, синхронно его сделать нельзя). Тесты, которым важен состав списка, явно
  ждут `await vm.RefreshEndpointsCommand.Execute()` — на это обновляются и существующие
  тесты, проверяющие состав `AvailableEndpoints` сразу после конструктора.
