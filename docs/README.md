...
- **Прошивка (`firmware/`)**
  ...
  - [`firmware/fluidnc-slow-motion-limits.md`](firmware/fluidnc-slow-motion-limits.md) — нижняя граница скорости FluidNC: порог `MINIMUM_FEED_RATE`, минимальная частота шага (`max(1.0, 2290 / steps_per_mm)`), мгновенное исполнение движений короче шага, ненадёжность `G93`. Разбор issue [#1715](https://github.com/bdring/FluidNC/issues/1715), [#1372](https://github.com/bdring/FluidNC/issues/1372), [#772](https://github.com/bdring/FluidNC/issues/772) и исходников прошивки + следствия для `TrajectoryCompiler`/`JogCommandFactory` и выбора редукции.
- **Протокол (`protocol/`)**
